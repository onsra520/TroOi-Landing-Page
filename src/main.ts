...
async function start():Promise<void>{
 ...
 const retry=document.querySelector<HTMLButtonElement>('#retry-3d');if(retry)retry.disabled=true;
 ...
 try{
  ...
  if(!current){assets.dispose();return;}
 ...
 }finally{starting=false;if(retry)retry.disabled=false;}
}
document.querySelector('#retry-3d')?.addEventListener('click',()=>void start());
window.addEventListener('trooi-retry',()=>void start());
window.addEventListener('pagehide',()=>current?.dispose(),{once:true});
void start();
