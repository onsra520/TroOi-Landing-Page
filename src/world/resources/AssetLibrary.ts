...
export class AssetLibrary implements AssetProvider {
  ...
  preload(): Promise<void> {
    if (this.disposed)
      return Promise.reject(new Error("AssetLibrary disposed"));
  ...
  configureTextures(anisotropy: number): void {
    for (const root of this.canonical.values())
      root.traverse((o) => {
  ...
  dispose(): void {
    ...
    this.disposed = true;
    this.canonical.forEach((root) => this.release(root));
  ...
  private async loadAll(): Promise<void> {
    ...
    await Promise.all(
      this.entries.map(async (entry) => {
        try {
          ...
          if (this.disposed) {
            this.release(root);
            throw new Error("AssetLibrary disposed");
          }
    ...
    const criticalFailures = this.entries.filter(
      (entry) => entry.critical && failures.has(entry.id),
    );
