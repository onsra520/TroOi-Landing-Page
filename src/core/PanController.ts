...
export class PanController {
  ...
  constructor(
    ...
    private readonly emit: (dx: number, dz: number) => void,
  ) {
    ...
    element.addEventListener("pointercancel", this.cancelEvent);
    element.addEventListener("lostpointercapture", this.lost);
    window.addEventListener("pointerup", this.up, true);
    window.addEventListener("pointercancel", this.cancelEvent, true);
  ...
  setTouchEnabled(value: boolean): void {
    ...
    this.element.style.touchAction = value ? "none" : "pan-y";
    if (!value) this.cancel();
  }
  update(dt: number): void {
    if (this.active !== null || !this.enabled || this.reduced?.matches) return;
    const step = Math.min(Math.max(dt, 0), 0.05);
    const decay = Math.exp(-12 * step);
    if (this.velocity.lengthSq() < 0.0001) {
      this.velocity.set(0, 0);
      return;
    }
    this.emit(
      (this.velocity.x * (1 - decay)) / 12,
      (this.velocity.y * (1 - decay)) / 12,
    );
  ...
  private down = (e: PointerEvent): void => {
    ...
    this.previous = this.project(e.clientX, e.clientY);
  ...
  private move = (e: PointerEvent): void => {
    ...
    if (
      !this.dragging &&
      Math.hypot(e.clientX - this.start.x, e.clientY - this.start.y) < 5
    )
      return;
    const point = this.project(e.clientX, e.clientY);
    if (!point || !this.previous) return;
    const dx = point.x - this.previous.x,
      dz = point.z - this.previous.z;
    const now = performance.now(),
      dt = Math.max((now - this.lastTime) / 1000, 1 / 120);
    this.velocity.set(dx / dt, dz / dt).clampLength(0, 30);
    this.lastTime = now;
    this.previous = point;
    this.dragging = true;
    this.emit(dx, dz);
  ...
  private up = (e: PointerEvent): void => {
    ...
    if (e.pointerId === this.active) this.stop(true);
  };
  private cancelEvent = (e: PointerEvent): void => {
    this.pointers.delete(e.pointerId);
    this.stop(false);
  };
  ...
  private stop(inertia: boolean): void {
    ...
    this.active = null;
    this.previous = null;
    if (!inertia || !this.dragging || this.reduced?.matches)
      this.velocity.set(0, 0);
  ...
  dispose(): void {
    ...
    this.element.removeEventListener("pointercancel", this.cancelEvent);
    this.element.removeEventListener("lostpointercapture", this.lost);
    window.removeEventListener("pointerup", this.up, true);
    window.removeEventListener("pointercancel", this.cancelEvent, true);
