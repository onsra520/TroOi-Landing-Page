...
export class Experience {
  ...
  start(): void {
    ...
    if (this.contextIsLost) throw new Error("WebGL context lost");
    if (!this.homeScene)
      throw new Error("Experience must be initialized before start");
  ...
  dispose(): void {
    ...
    this.disposed = true;
    this.renderer.instance.domElement.removeEventListener(
      "webglcontextlost",
      this.contextLost,
    );
    this.renderer.instance.domElement.removeEventListener(
  ...
  private async prepareScene(): Promise<void> {
    ...
    if (this.disposed) throw new Error("Experience disposed");
    if (this.contextIsLost) throw new Error("WebGL context lost");
    this.assets.configureTextures?.(
      Math.min(
        this.renderer.instance.capabilities.getMaxAnisotropy(),
        this.sizes.width < 768 ? 4 : 8,
      ),
    );
    ...
    this.pan = new PanController(
      ...
      (x, z) => this.homeScene?.town.panBy(x, z),
    );
    this.unlock = this.camera.onInputChange((enabled) =>
      this.pan?.setEnabled(enabled),
    );
  ...
  private readonly contextRestored = (): void => {
    ...
    window.dispatchEvent(new Event("trooi-retry"));
  };
  ...
  private readonly toggleExplore = (): void => {
    ...
    if (this.explore)
      this.explore.textContent = enabled ? "Dừng khám phá" : "Khám phá bản đồ";
    this.pan?.setTouchEnabled(enabled);
  };

  panBy(x: number, z: number): void {
    this.homeScene?.town.panBy(x, z);
    this.homeScene?.town.update(0, this.time.elapsed);
  }
  snapshot() {
    ...
    return {
      ...
      ...this.homeScene?.town.snapshot(),
      poolSize: this.homeScene?.town.poolSize,
      rootY: this.homeScene?.town.root.position.y,
    };
  }
